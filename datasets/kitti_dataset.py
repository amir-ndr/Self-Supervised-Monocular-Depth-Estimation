...
class KITTIRAWDataset(KITTIDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for date in self.dates:
            ...
            if os.path.isfile(calib_filename):
                ...
                # print(cam2cam['P_rect_00'].reshape([3, 4]))
    ...
    def get_depth(self, folder, frame_index, side, do_flip):
        ...
        depth_gt = np.array(Image.fromarray(depth_gt).resize(size = self.default_full_res_shape))
...
class KITTIDepthDataset(KITTIDataset):
    ...
    def get_depth(self, folder, frame_index, side, do_flip):
        ...
        depth_gt = np.array(depth_gt.resize(self.default_full_res_shape, pil.NEAREST)).astype(np.float32) / 256
