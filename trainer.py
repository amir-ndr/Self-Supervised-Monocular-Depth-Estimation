...
class Trainer:
    def __init__(self, options, backbone = 'resnet18'):
        self.opt = options
        ...
        if backbone == 'resnet18':
            ...
            self.models["encoder"] = networks.ResnetEncoder(18, False)
            self.models["encoder"].to(self.device)
            print(count_parameters(self.models["encoder"]))
            self.parameters_to_train += list(self.models["encoder"].parameters())

            self.models["depth"] = networks.DepthDecoder(
                self.models["encoder"].num_ch_enc, self.opt.scales)
            self.models["depth"].to(self.device)
            print(count_parameters(self.models["depth"]))
            self.parameters_to_train += list(self.models["depth"].parameters())

        elif backbone == 'STDC1':

            self.models["encoder"] = networks.STDCNet813()
            self.models["encoder"].to(self.device)
            print(count_parameters(self.models["encoder"]))
            self.parameters_to_train += list(self.models["encoder"].parameters())

            self.models["depth"] = networks.DepthDecoder_STDC(
                self.models["encoder"].num_ch_enc, self.opt.scales)
            self.models["depth"].to(self.device)
            print(count_parameters(self.models["depth"]))
            self.parameters_to_train += list(self.models["depth"].parameters())
            ...
            self.models["encoder"] = networks.STDCNet1446()
            self.models["encoder"].to(self.device)
            print(count_parameters(self.models["encoder"]))
            self.parameters_to_train += list(self.models["encoder"].parameters())
            ...
            self.models["encoder"] = networks.mydarknet()
            self.models["encoder"].to(self.device)
            print(count_parameters(self.models["encoder"]))
            self.parameters_to_train += list(self.models["encoder"].parameters())

            self.models["depth"] = networks.DepthDecoder_nano(
                self.models["encoder"].num_ch_enc, self.opt.scales)
            self.models["depth"].to(self.device)
            print(count_parameters(self.models["depth"]))
            self.parameters_to_train += list(self.models["depth"].parameters())
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.height == '96':
            size_mode = '96_320'
        elif self.opt.height == '192':
            size_mode = '192_640'

        if self.backbone in ['resnet18', 'STDC2', 'STDC1']:

            x = self.models["encoder"](inputs["color_aug", 0, 0])
            # print(x.shape)
            depth_encoder_features = x
            outputs.update(self.models["depth"](depth_encoder_features, size_mode=size_mode))

        elif self.backbone == 'darknet_nano':
            x, y = self.models["encoder"](inputs["color_aug", 0, 0])
            # print(x.shape)
            depth_encoder_features = x
            outputs.update(self.models["depth"](depth_encoder_features, y, size_mode=size_mode))
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            _, depth = disp_to_depth(disp, self.opt.min_depth, 100)
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                # in0 = torch.permute(inputs[("color", 0, 0)][0], (1, 2, 0)).detach().numpy()
                # in0 = cv2.resize(in0, (500, 500))
                # out1 = torch.permute(outputs[("color", frame_id, 0)][0], (1, 2, 0)).detach().numpy()
                # out1 = cv2.resize(out1, (500, 500))
                # cv2.putText(out1, '{}'.format(frame_id), (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                # cobined_image = np.hstack((in0, out1))
                # cv2.imshow('im', cobined_image)
                # cv2.waitKey(0)
            ...
            # in0 = torch.permute(inputs[("color", 0, 0)][0], (1, 2, 0)).detach().numpy()
            # inMinus1 = torch.permute(inputs[("color", -1, 0)][0], (1, 2, 0)).detach().numpy()
            # inPlus1 = torch.permute(inputs[("color", 1, 0)][0], (1, 2, 0)).detach().numpy()
            # out1 = torch.permute(outputs[("color", 1, 0)][0], (1, 2, 0)).detach().numpy()
            # outMinus1 = torch.permute(outputs[("color", -1, 0)][0], (1, 2, 0)).detach().numpy()
            # inMinus1 = cv2.resize(inMinus1, (300, 500))
            # inPlus1 = cv2.resize(inPlus1, (300, 500))
            # in0 = cv2.resize(in0, (300, 500))
            # out1 = cv2.resize(out1, (300, 500))
            # outMinus1 = cv2.resize(outMinus1, (300, 500))
            # cv2.putText(out1, "out(frame 1)" , (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,(0, 0, 255), 2)
            # cv2.putText(inMinus1, "source(-1)", (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            # cv2.putText(in0, "target", (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            # cv2.putText(inPlus1, "source(1)", (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            # cv2.putText(outMinus1, "out(frame -1)", (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            # cobined_image = np.hstack((inMinus1,inPlus1, in0, out1, outMinus1))
            # cv2.imshow('im', cobined_image)
            # cv2.waitKey(0)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                # target_new = torch.permute(color[0], (1, 2, 0)).detach().numpy()
                # target_new = cv2.resize(target_new, (500, 500))
                # out1 = torch.permute(outputs[("color", frame_id, scale)][0], (1, 2, 0)).detach().numpy()
                # out1 = cv2.resize(out1, (500, 500))
                # cv2.putText(out1, '{}'.format(frame_id), (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                # cobined_image = np.hstack((target_new, out1))
                # cv2.imshow('im', cobined_image)
                # cv2.waitKey(0)
            ...
            if self.opt.disable_masking:
                ...
                for frame_id in self.opt.frame_ids[1:]:        #set all output mask to 1
                ...
                if not self.opt.disable_occlusion_mask_from_image_boundary:  #image boundry
                    for frame_id in self.opt.frame_ids[1:]:
                        outputs[("mask", frame_id, scale)] *= \
                            outputs[("occlusion_mask_from_image_boundary", frame_id, scale)]

                if not self.opt.disable_occlusion_mask_from_photometric_error:  #min_repro
                ...
                if not self.opt.disable_stationary_mask_from_photometric_error:  # M_a s
                    for frame_id in self.opt.frame_ids[1:]:
                ...
                if not self.opt.disable_outlier_mask_from_photometric_error:    #M_ol
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        # depth_pred = torch.clamp(depth_pred, self.dataset.min_depth, self.dataset.max_depth).detach()

        depth_gt = inputs[("depth_gt", 0)]
        depth_gt = torch.permute(depth_gt, (0, 1, 3, 2))
        # print(depth_gt.size())
        # print(depth_pred.shape[2], depth_pred.shape[3])

        # depth_gt = F.interpolate(depth_gt, (depth_pred.shape[2], depth_pred.shape[3]), mode="bilinear", align_corners=False)
        mask = depth_gt > 0
    ...
    def save_best_model(self):
        ...
        """
        save_folder = os.path.join(self.log_path, "best_model", "weights")
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
